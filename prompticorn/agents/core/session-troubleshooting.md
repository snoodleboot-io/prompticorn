<!-- path: prompticorn/prompts/agents/core/core-session-troubleshooting.md -->
...
**Symptoms:**
- No files in `.prompticorn/sessions/` directory
...
ls -la .prompticorn/sessions/session_*.md
...
git log --all -- .prompticorn/sessions/
...
grep -l "branch: \"feat/PROJ-123-auth\"" .prompticorn/sessions/session_*.md
```
...
1. **Identify which session is current:**
   ...
   ls -lt .prompticorn/sessions/session_*.md | head -3
   ```
...
3. **Delete duplicates:**
   ...
   rm .prompticorn/sessions/session_older_date_*.md
   ```
...
1. **Fix the YAML carefully:**
   ...
   vim .prompticorn/sessions/session_20260304_k7m9x1.md
...
cat .prompticorn/sessions/session_*.md
...
```bash
# Check session age
ls -l .prompticorn/sessions/session_*.md
...
# Archive old session
mkdir -p .prompticorn/sessions/archive/
mv .prompticorn/sessions/session_old_date_*.md .prompticorn/sessions/archive/
...
mkdir -p .prompticorn/sessions/archive/
mv .prompticorn/sessions/session_*.md .prompticorn/sessions/archive/
...
grep -A 10 "## Mode History" .prompticorn/sessions/session_*.md
...
mkdir -p .prompticorn/sessions/archive/
mv old_session.md .prompticorn/sessions/archive/
```
