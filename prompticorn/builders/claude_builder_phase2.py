...
class ClaudeBuilder(Builder):
    ...
    def build(
        ...
        if options.include_subagents and agent.subagents:
            for subagent_name in agent.subagents:
                ...
                # TODO: Load full subagent data from registry
        ...
        if options.include_workflows and agent.workflows:
            for workflow_name in agent.workflows:
                ...
                # TODO: Load full workflow data from workflow loader
    ...
    def _render_agent_file(
        ...
        template_data = {
            ...
            "workflow_steps": self._extract_workflow_steps(agent.workflows[0])
            if agent.workflows
            else [],
            "subagents": self._prepare_subagents_data(agent.subagents) if agent.subagents else [],
            "skills": self._prepare_skills_data(agent.skills) if agent.skills else [],
    ...
    def _extract_workflow_steps(self, workflow_name: str) -> list[str]:
        ...
        """
        # Default steps - these would be loaded from actual workflow files
        return [
            ...
            subagents.append(
                {
                    ...
                    "purpose": f"Specialized for {name} tasks",
                    "file_name": file_name,
                    "when_to_use": f"When you need focused {name} assistance",
                }
            )
        return subagents
