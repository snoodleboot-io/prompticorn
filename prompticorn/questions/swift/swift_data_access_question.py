"""Swift data access / query layer question for backend folders."""

from prompticorn.questions.base.question import Question


class SwiftDataAccessQuestion(Question):
    """Swift data access / query layer - multi-select, backend-only.
    ...
    def options(self) -> list[str]:
        return ["Fluent (Vapor)", "GRDB", "Core Data", "Raw SQL"]

    @property
    def option_explanations(self) -> dict[str, str]:
        return {
            "Fluent (Vapor)": "Vapor's async ORM with multiple database drivers",
            "GRDB": "SQLite toolkit with records, migrations, and query interface",
            "Core Data": "Apple's object graph and persistence framework",
            "Raw SQL": "Hand-written SQL via a database driver",
        }
    ...
    def default_indices(self) -> set[int]:
        """Default selection for multi-select: Fluent (Vapor) (0)."""
        return {0}
