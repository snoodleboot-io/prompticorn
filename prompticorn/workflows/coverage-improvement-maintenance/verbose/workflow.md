...
- `prompticorn/ui/_selector.py` - 32% (138 lines)
- `prompticorn/ui/input/windows.py` - 25% (62 lines)
- `prompticorn/ui/input/unix.py` - 20% (66 lines)
- `prompticorn/ui/pipeline/orchestrator.py` - 24% (41 lines)
- `prompticorn/registry.py` - 53% (117 lines)
...
pytest --cov=prompticorn --cov-report=html -q
...
# Export for analysis
pytest --cov=prompticorn --cov-report=term-missing -q > coverage-analysis.txt
```
...
pytest tests/unit/[module]/test_[file].py --cov=prompticorn.[module] --cov-report=term-missing
...
pytest --cov=prompticorn --cov-report=term-missing

# JSON for processing
pytest --cov=prompticorn --cov-report=json
```
