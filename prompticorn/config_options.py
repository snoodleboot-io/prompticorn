"""Configuration options management for the prompticorn update command.

This module provides data structures and functions for managing configuration
options that can be updated interactively via the `prompticorn update` command.
...
Configuration is loaded from YAML files in prompticorn/configurations/
"""
...
class ConfigOptionsRegistry:
    ...
    Configuration is loaded from prompticorn/configurations/config_options.yaml
    """
...
class ConfigOption:
    ...
    `prompticorn update` command. Each option has a key (used in the config file),
